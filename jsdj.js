/*
 ...
 变量：jsdjCK='xxxx@12345678910#abcdefg '  多个账号用 @ 或者 换行 分割
 定时一天三次
 cron 10 8,10,19 * * *
 */
 const $ = new Env('江山多娇')
...
//  let replytxt = replytxtArr[randReply]
 let cookiesArr = [],
   ...
   channelId = [
    ...
    "637c91d5b40eef5a490bcea9", 
    "5d3fea3ab198500f695bdec4", 
    "5d3fe981b198500f695bdebf", ]
 cookie = ($.isNode() ? process.env.jsdjCK : $.getdata("jsdjCK")) || ``
 helpAu = ($.isNode() ? process.env.jsdjhelp : $.getdata("jsdjhelp")) || true
 //开始===============================================================================================
 !(async () => {
     ...
     for (let i = 0; i < cookiesArr.length; i++) {
       if (cookiesArr[i]) {
         ...
         console.log(`\n******开始【🐳江山多娇账号${$.index}】${$.nickName}|${$.mobile}*********\n`);
         await main()//调用任务列表
       }
     }
     if ($.isNode() && message) {
 ...
 async function main() {
   if ($.userId) {
     ...
     if (helpAu == true) {
       //console.log(`【环境变量：jsdjhelp 默认为True 前往助力作者】`)
       await invite()
     }
     console.log(`【查询账号信息】`)
 ...
 async function article() {
   let body = `?channel_id=${channelId[Math.floor(Math.random()*channelId.length)]}&isDiFangHao=false&is_new=true&list_count=${Math.floor(Math.random()*5+1)*10}&size=10&start=${Date.now()}`
   return new Promise(resolve => {
