/*
 ...
 变量：ydkc_ck='xxxx@12345678910#abcdefg '  多个账号用 @ 或者 换行 分割
 定时一天三次
 cron 10 8,10,19 * * *
 646f27d5c790b0698b5ac6ed
 */
 const $ = new Env('运动柯城')
 ...
 let cookiesArr = [],
   message = "",
   channelId = [
    "5d60c2931b011b2a0fbb9c4d", 
    "6229a95cad61a429c691c707", 
    "63490a34fe3fc1680f581e1f", 
    "5d60bf44b1985030db8625f4", 
    "5d60be1db1985030db8625f2", 
    "5d60bdf21b011b2a0fbb9c4a"]
 cookie = ($.isNode() ? process.env.ydkc_ck : $.getdata("ydkc_ck")) || ``
 helpAu = ($.isNode() ? process.env.ydkchelp : $.getdata("ydkchelp")) || true
 //开始===============================================================================================
 !(async () => {
 ...
 async function main() {
   if ($.userId) {
    ...
    // }
     if (helpAu == true) {
       //console.log(`【环境变量：ydkchelp 默认为True 前往助力作者】`)
       await invite()
     }
     console.log(`【查询账号信息】`)
   ...
   switch (type) {
     ...
     case '313': //邀请
      ...
      //  break;
       ...
       for (j = 0; j < num && j < $.acticleList.length; j++) {
         console.log(`去分享：${$.acticleList[j].list_title}`)
         await sqshare($.acticleList[j].id)
         await $.wait(1500)
       }
       break;
       case '740': //社区帖子评论
       for (j = 0; j < num && j < $.acticleList.length; j++) {
         console.log(`去评论：${$.acticleList[j].list_title}`)
         await sqcomment($.acticleList[j].id)
         await $.wait(1500)
        }
       break;
      //  case '677': //社区帖子点赞
  ...
  * 获取社区文章
  */
async function forum() {//社区文章
    let body = `?forum_id=507`
    return new Promise(resolve => {
      $.get((taskUrl("/api/forum/thread_list", body)), async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`)
            console.log(`${$.name} API请求失败，请检查网路重试`)
            ...
            if (data) {
              ...
              //console.log(JSON.stringify(data));
              if (data.code === 0) {
                $.acticleList = data.data['thread_list']
            } else {
                console.log(data.message)
              }
            } else {
  ...
  * 分享社区资讯给好友
  */
 async function sqshare() {
  let body = `member_type=14`
  return new Promise(resolve => {
    $.post((taskPostUrl("/api/user_mumber/doTask", body)), async (err, resp, data) => {
      try {
        if (err) {
...
//====================================================================================================
  ...
  * 社区评论
  */
 async function sqcomment(id) {
    let body = `thread_id=${id}&type=1`
    return new Promise(resolve => {
      $.post((taskPostUrl("api/forum/comment_list", body)), async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err}`)
            console.log(`${$.name} API请求失败，请检查网路重试`)
 ...
 function taskPostUrl(type, body) {
   ...
   return {
     ...
     headers: {
       ...
       'Host': 'vapp.tmuyun.com',
       'Connection': 'Keep-Alive',
       'X-Requested-With': 'com.zjonline.kecheng',
       //"Content-Type": `application/json; charset=UTF-8`,
       'User-Agent': `1.2.2;${requestid};iPad13,4;IOS;16.2;Appstore`
     },
   }
