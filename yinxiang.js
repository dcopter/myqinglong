/*
 ...
 变量：qxiangCK='xxxx@12345678910#abcdefg '  多个账号用 @ 或者 换行 分割
 定时一天三次
 cron 10 8,10,19 * * *
 */
 const $ = new Env('鄞响')
 ...
 let cookiesArr = [],
   ...
   channelId = [
    ...
    "63574148fe3fc1680f583f90", 
    "63574156ad61a45c31a182c4", 
    "63691516fe3fc1747d4d37b6", 
    "6357495df0edc004ccfe1f5f",
    "63758ce2ad61a40b77d50485",
    "6375cc46fe3fc1747d4d86b5",
]
 cookie = ($.isNode() ? process.env.qxiangCK : $.getdata("qxiangCK")) || ``
 helpAu = ($.isNode() ? process.env.qxhelp : $.getdata("qxhelp")) || true
 //开始===============================================================================================
 !(async () => {
 ...
 async function main() {
   if ($.userId) {
     ...
     if (helpAu == true) {
       //console.log(`【环境变量：qxhelp 默认为True 前往助力作者】`)
       await invite()
     }
     console.log(`【查询账号信息】`)
  ...
  */
  async function credential_auth() {
   let url = {
     ...
     body: `client_id=10014&password=${encodeURIComponent($.pwd)}&phone_number=${$.mobile}`,
     headers: {
 ...
 async function article() {
   let body = `?channel_id=${channelId[Math.floor(Math.random()*channelId.length)]}&isDiFangHao=false&is_new=true&list_count=0&size=10&start=${Date.now()}`
   return new Promise(resolve => {
